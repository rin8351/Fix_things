...
class Time_fix:
    def __init__(self):
        ...
        self.frame_rf.pack()
        if not os.path.exists('hom.pickle'):
            save_object(PickClass({}), 'hom.pickle')
        if not os.path.exists('comm.pickle'):
            save_object(PickClass({}), 'comm.pickle')
        data_com = load_object("comm.pickle").param
        if data_com == '':
            self.comm = {}
        else:
            self.comm = data_com
        self.save_zap = []
        if not os.path.exists('archive.pickle'):
            save_object(PickClass({}), 'archive.pickle')
        self.time_zan = load_object('archive.pickle').param
        if self.time_zan =='':
            self.time_zan = {}
        self.spisok_del = []
        for i in self.time_zan :
            for j in self.time_zan[i] :
                if '_d' in j:
                    self.spisok_del.append(j)
        self.hom = load_object("hom.pickle").param
        if self.hom == {} or self.hom == '':
            self.hom = ''
        self.zan = []
        self.week = []
        self.today = datetime.date.today()
        for i in range(7):
            self.week.append(str(self.today - datetime.timedelta(days=i)))
        if not os.path.exists('plan.pickle'):
        ...
        if str(self.need_date) != str(self.today):
            msg = "Данные за дату "+str(self.need_date)+" не сохранены"
            messagebox.showinfo("ВНИМАНИЕ", msg)
    ...
    def canvass(self,k_save):
        ...
        self.but_plan = Button(self.r2 , text='Сохранить\n план', command = self.plan, font=self.font, width=12, height=2, bg='#ffffff')
        ...
        if data != {}:
            self.table2()
            self.but_plan.configure(text='Сохранить\n план', bg='#84B1C2')
            for k in data:
                self.need_date = k
        else:
            self.need_date = self.today
    ...
    def table(self):
        ...
        for i in self.time_zan:
            ...
            if self.spisok_del != []:
                ...
                spisok_del2=[]
                for d in self.time_zan[i]:
                    if d in self.spisok_del:
                        spisok_del2.append(d)
                if spisok_del2 != []:
            ...
            for j in self.time_zan[i]:
                if '_d' not in j:
                    ...
                    if j in self.comm:
                        ToolTip(self.bt, self.comm[j])
                    lb = Label(self.inner_frame, text=self.count_time(j), font=self.font, width=5, height=1, bg='#ffffff')
                    lb.grid(row=x, column=1,sticky=W, pady=4, padx=4)
                    lb2 = Label(self.inner_frame, text=self.count_in_week(j), font=self.font, width=5, height=1, bg='#ffffff')
                    lb2.grid(row=x, column=2,sticky=W, pady=4, padx=4)
                    x +=1
                self.count_all_m += self.count_time(j)
                self.count_all_w += self.count_in_week(j)
                x+=1
            x +=1
            self.lb_all_m.configure(text=self.count_all_m)
            self.lb_all_w.configure(text=self.count_all_w)

    def count_time(self,i):
        for j in self.time_zan:
            if i in self.time_zan[j]:
                return len(self.time_zan[j][i])

    def count_in_week(self,i):
        week_this = []
        today = datetime.date.today()
        day_of_week = today.weekday()
        week_this.append(today - datetime.timedelta(days=day_of_week))
        for w in range(1, 7):
            week_this.append(week_this[0] + datetime.timedelta(days=w))
        for j in self.time_zan:
            if i in self.time_zan[j]:
                count_d=0
                for k in week_this:
                    if str(k) in self.time_zan[j][i]:
                        count_d+=1
                return count_d
    ...
    def save_to_pickle(self):        
        ...
        self.save_zap = []
        if self.need_date == chosen_date:
            save_object(PickClass(self.time_zan), 'plan.pickle')
            self.but_plan.configure(text='Сохранить\n план', bg='#ffffff')
            self.r2.destroy()
            self.kartina_save = self.image_dict['plan']
            self.canvass(self.kartina_save)
            ...
            if self.zan !=[]:
                self.table2()
                self.but_plan.configure(text='Сохранить\n план', bg='#84B1C2')
                self.variable0.set(self.need_date)

    def otmena(self):
        if self.save_for_undo !=[]:
            self.time_zan=copy.deepcopy(self.save_for_undo)
            msg = "Отмена"
            messagebox.showinfo("Изменения отменены", msg)
            self.save_to_pickle()
            self.save_for_undo=[]
    ...
    def plan(self):
        ...
        if self.zan == []:
            ...
            st[self.variable0.get()] = self.zan
            self.but_plan.configure(bg='#84B1C2')
            self.kartina_save=self.image_dict['plan2']
        save_object(PickClass(st), 'plan.pickle')
        self.r2.destroy()
        self.canvass(self.kartina_save)
    ...
    def household_wind(self):
        ...
        self.but_hom = Button(self.frame_hom, text="Сохранить",font=self.font, bg='#0076A3',fg='#ffffff', command=lambda: self.add_del_hom("Сохранить",'house2','#84B1C2'))
        self.but_hom.grid(row=2, column=1, padx=4, pady=4)
        self.but_del_hom = Button(self.frame_hom, text="Удалить",font=self.font, bg='#0076A3',fg='#ffffff', command=lambda: self.add_del_hom("Удалить",'house','#ffffff'))
        self.but_del_hom.grid(row=3, column=1, padx=4, pady=4)
        self.text_hom.insert(END, self.hom)

    def add_del_hom(self, text, img,color):
        if text == "Сохранить":
    ...
    def comment (self):
        self.frame_com = Frame(self.tab4)
